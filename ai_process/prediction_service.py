#!/usr/bin/env python3
"""
Continuous 24/7 Prediction Service for liquidLapse

This service runs independently and continuously makes predictions using the latest
trained models on recent heatmap snapshots. It saves comprehensive prediction metadata
for later validation against ground truth data.

Features:
- Auto-discovery of latest trained models
- Configurable prediction intervals
- Robust error handling and recovery
- Comprehensive metadata storage
- Session-based prediction organization
- Optional model memory persistence

Usage:
    python prediction_service.py [options]
    
Service Management:
    Use prediction_service.sh to manage as a background service
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="liquidLapse Continuous Prediction Service")
    ...
    parser.add_argument('--daemon', action='store_true',
                        help="Run as daemon (used by service script)")
    ...
    # Create status and PID files
    pid_file = os.path.join(base_dir, DEFAULT_PID_FILE)
    status_file = os.path.join(base_dir, DEFAULT_STATUS_FILE)
    
    log_message("=" * 60)
    log_message("liquidLapse Prediction Service Starting")
    ...
    write_status('starting', pid_file, status_file)
    
    # Main service loop
    prediction_interval = config['prediction'].get('prediction_interval', 300)
    log_message(f"Starting prediction loop with {prediction_interval}s interval")
    
    write_status('running', pid_file, status_file)
    ...
    while running:
        try:
            cycle_count += 1
            log_message(f"--- Prediction Cycle #{cycle_count} ---")
            
            success = prediction_cycle(config, directories, device)
            
            if success:
                last_success = datetime.now()
                log_message("Prediction cycle completed successfully")
            else:
                log_message("Prediction cycle failed, will retry next interval", "WARN")
            
            # Wait for next cycle
            if running:  # Check again in case we received shutdown signal
                log_message(f"Waiting {prediction_interval} seconds until next prediction...")
            ...
            log_message(f"ERROR: Unexpected error in main loop: {e}", "ERROR")
            log_message(f"Traceback: {traceback.format_exc()}", "DEBUG")
            
            if running:
                log_message("Continuing service after error...", "INFO")
                time.sleep(10)  # Brief pause before retry
    
    # Cleanup
    write_status('stopped', pid_file, status_file)
    log_message("Prediction service stopped gracefully")
